...
export default function Sidebar({
  ...
  const { toggleDayExpand, toggleTopicExpand, handleSubtopicClick } = actions;
  ...
  return (
    <div className="w-1/4 border-r p-4 overflow-y-auto bg-slate-50">
      ...
      <div className="mb-6 p-4 bg-indigo-600 rounded-2xl text-white shadow-lg">
        <h2 className="text-lg font-bold truncate">{metaData.subject || "Study Material"}</h2>
          ...
          return (
            <div key={dayIndex} className="mb-2">
              ...
              >
                <div className="flex items-center gap-3">
                  <div
                    className={`w-2 h-2 rounded-full ${
                      dayExpanded ? "bg-indigo-600" : "bg-gray-300"
                    }`}
                  />
                ...
                {dayExpanded ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
              </button>

              {/* Topics & Subtopics - Only show if day is expanded */}
              {dayExpanded && dayItem.topics?.map((topic, topicIndex) => {
                const topicExpanded = isTopicExpanded(dayIndex, topicIndex);
                
                return (
                  <div key={topicIndex} className="mt-2 ml-4 space-y-1 border-l-2 border-indigo-100 pl-2">
                    {/* Topic Button */}
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 text-sm font-semibold flex justify-between items-center ${
                        topicExpanded ? "text-indigo-600" : "text-gray-600 hover:text-indigo-600"
                      }`}
                      disabled={loadingContent}
                    >
                    ...
                    {topicExpanded && topic.subtopics?.map((subtopic, subtopicIndex) => {
                      const isSelected = selectedSubtopic?.title === subtopic.title && 
                                        selectedSubtopic?.currentDay === dayItem.day;
                      
                      return (
                        <button
                          key={subtopicIndex}
                          onClick={() => handleSubtopicClick(dayItem.day, topicIndex, subtopicIndex)}
                          ...
                          disabled={loadingContent}
                        >
                          <span className={subtopic.completed ? "line-through opacity-50" : ""}>
                            {subtopic.title}
                          </span>
                          {subtopic.completed && <Check size={12} className="text-emerald-600" />}
                        </button>
                      );
                    })}
                  </div>
